...
"""
    Builds a cumulative claim count triangle with accident months as rows and 
    ...
    Output:
    a pandas datframe representing a claims count triangle with the number of 
    claims from an Accident Month progressing by development month periods 
    until the evaluation date
"""

def claim_count_triangle_at(claims_df, eval_date, max_dev_month=None):
    ...
    # Populating incremental claim coutns by (accident_month, dev_month):
    # After bucketing all claims with the same accident and development month, 
    # their claim conuts are summed so there's only 1 entry per 
    # [accident month, development month] pair
    incremental_counts = paid.groupby(['accident_month', 'dev_month']).size()
    ...
    # is filled with 0
    incremental_counts = incremental_counts.unstack(fill_value=0)
    ...
    incremental_counts = incremental_counts.reindex(index=sorted_accident_months, columns=range(0, max_dev + 1), fill_value=0)

    # Taking the cumulative sum across development months for all rows
    cumulative_sums = incremental_counts.cumsum(axis=1)
